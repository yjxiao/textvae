...
class Corpus(object):
    def __init__(self, datadir, min_n=2):
        self.min_n = min_n
        filenames = ['empty.txt', 'train.txt', 'valid.txt', 'test.txt']
        self.datapaths = [os.path.join(datadir, x) for x in filenames]
        self._construct_vocab()
        self.train_data = Data(self.datapaths[0:2], self.word2idx)
        self.valid_data = Data(self.datapaths[2:3], self.word2idx)
        self.test_data = Data(self.datapaths[3:4], self.word2idx)
